...
app.get("/query", function (req, res) {
    ...
    con.query(sqlStatement, [dest_name, dest_name], function (err, result) {
        try {
            ...
            {
                res.send("Destination Not available");
            }
        } catch (err){
...
app.post("/query/delete", function (req, res) {
    ...
    if (sess.username) {
        if (sess.user_type == "admin") {
            ...
            if(destination)
            {
                query_delete1(destination)
                .then(function(rows){
                    return query_delete2(destination);
                }).catch(function(err){
                    res.send(err);
                }
                )
                .then(function(rows){
                    var sqlStatement = "delete from destination where destination.dest_name = ?;";
                    console.log(sqlStatement);
                    con.query(sqlStatement,[destination],function (err, result) {
                        try {
                            ...
                            res.send("Some error while doing delete query in destination1");
                        }
                    });
                    }
                ).catch((err) => setImmediate(() => { throw err; }));
                    ...
                    {
                        res.send("Destination is Not Added.");
                    }
        } else {
 ...
 function query_delete1(destination)
 {
     return new Promise(function(resolve,reject){
        var sqlStatement = "delete from payments where payments.package_id in (select package.package_id from package where package.dest_id in ( select destination.dest_id from destination where destination.dest_name = ?))";
        console.log(sqlStatement);
        con.query(sqlStatement,[destination],function (err, result) {
            if(err)
                return reject(err);
...
function query_delete2(destination)
 {
     return new Promise(function(resolve,reject){
        var sqlStatement = "delete from package where package.dest_id in ( select destination.dest_id from destination where destination.dest_name = ?)";
        console.log(sqlStatement);
        con.query(sqlStatement,[destination],function (err, result) {
            if(err)
                return reject(err);
...
app.post("/query", function (req, res) {
    ...
    if (sess.username) {
        if (sess.user_type == "admin") {
            ...
            if( destImageURL && information && destination)
            {
